...
import { deleteBlocks, getAllBlocks } from '../../../../store/actions/userActions';
            ...
            buttons: [
                {
                    label: 'Yes',
                    onClick: () => {
                        dispatch(deleteBlocks(id)).then((result) => {
                            setDelLoading(false)
    ...
    return (
        <>
            <Box sx={{ padding: '30px' }}>
                ...
                <AddBlocks open={open} close={() => setOpen(false)} createSuccess={getBlocks} />
            </Box>
        </>
    )
}
