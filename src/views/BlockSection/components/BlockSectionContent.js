import { AppBar, Box, Button, Checkbox, FormControlLabel, 
    IconButton, InputAdornment, Menu, MenuItem, TextField, 
    Toolbar, Typography, styled, Tabs,Tab } from '@mui/material'
  ...
  return (
    <StyledRoot >
        <Box sx={{px:10}}>
            <Box sx={{display:'flex'}}>
                <Box flex={1}>
                <Box sx={{ border: '1px solid rgba(0,0,0,0.1)', height: '100vh',p: 2, }}>
          <TextField
            variant="outlined"
            placeholder="Search..."
            size='small'
          ...
          >
            ...
            <MenuItem>
              ...
              <Button variant="outlined" sx={{m:3, ml:8}}>Apply filters</Button>
                ...
                <Box flex={3}>
                    <AppBar position='static' sx={{background:'#fff', color:'#000'}} elevation={1}>
                        <Toolbar >
                        <Tabs value={tabValue} onChange={handleTabChange}>
                        <Tab icon={<Inventory2Icon/>} iconPosition='start' label="Products" />
                        <Tab label="Models" />
                        <Tab label="3d blocks" />
                        <Tab label="2d blocks" />
                        <Tab label="Materials" />
